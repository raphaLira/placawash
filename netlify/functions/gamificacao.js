...
exports.handler = async (event) => {
  ...
  if (event.httpMethod==='GET' && q.action==='resumo') {
    try {
      ...
      return { statusCode:200, headers, body: JSON.stringify({ streak:0, metaHoje:8, progressoHoje:0, totalHoje:0, conquistas:[] })};
    }
  }

  if (event.httpMethod==='POST' && q.action==='checkin') {
    try {
      const hoje  = hojeISO();
      ...
      if (!existente) {
        ...
        return { statusCode:200, headers, body: JSON.stringify({ streak:1, novo:true })};
      }

      if (existente.ultima_atividade === hoje)
        return { statusCode:200, headers, body: JSON.stringify({ streak: existente.streak_atual, novo:false })};
      ...
      return { statusCode:200, headers, body: JSON.stringify({ streak:novoStreak, recorde:novoRecorde, novo:true })};
    } catch(e) {
      return { statusCode:200, headers, body: JSON.stringify({ streak:0, novo:false })};
    }
  }
